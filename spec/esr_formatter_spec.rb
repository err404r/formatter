require 'pty'
require 'nokogiri'
require 'esr_formatter'

describe ESRFormatter do
  EXAMPLE_DIR = File.expand_path('../example', __dir__)
  FORMATTER_FILE = File.expand_path('../lib/esr_formatter.rb', __dir__)
  SCHEMA_FILE = File.expand_path('../misc/JUnit.xsd', __dir__)

  let(:formatter_arguments) { ['--format', 'ESRFormatter', '--require', FORMATTER_FILE.to_s] }
  let(:extra_arguments) { [] }
  ...
  def safe_pty(command, directory)
    sio = StringIO.new
    begin
      PTY.spawn(*command, chdir: directory) do |r, _w, pid|
        begin
  ...
  def execute_example_spec
    command = ['rspec', *formatter_arguments, color_opt, *extra_arguments]
  ...
  let(:testsuite) { doc.xpath('/testsuites/testsuite').first }
  let(:testcases) { doc.xpath('/testsuites/testsuite/testcase') }
  let(:successful_testcases) { doc.xpath('/testsuites/testsuite/testcase[success]') }
  let(:pending_testcases) { doc.xpath('/testsuites/testsuite/testcase[skipped]') }
  let(:failed_testcases) { doc.xpath('/testsuites/testsuite/testcase[failure]') }
  let(:shared_testcases) { doc.xpath("/testsuites/testsuite/testcase[contains(@name, 'shared example')]") }
  let(:failed_shared_testcases) { doc.xpath("/testsuites/testsuite/testcase[contains(@name, 'shared example')][failure]") }
  ...
  it 'correctly describes the test results', aggregate_failures: true do
    # it should match xml schema

    schema = Nokogiri::XML::Schema(File.read(SCHEMA_FILE))
    expect(schema.validate(doc).size).to eql(0)
    ...
    successful_testcases.each do |testcase|
      ...
      expect(child.name).to eql('success')
